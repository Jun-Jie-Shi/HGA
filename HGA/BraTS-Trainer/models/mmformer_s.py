...
class Model(nn.Module):
    ...
    def forward(self, x, mask, target=None, temp=1.0, uni=None):
        B = x.size(0)
        device = x.device
        ...
        if self.is_training:
            ...
            if self.use_passion:                                
                ...
                if uni:
                    sep_loss += mask * masks_mod0 * (softmax_weighted_loss_bs(F.softmax(fuse_pred_flair, dim=1), target, num_cls=num_cls) + dice_loss_bs(F.softmax(fuse_pred_flair, dim=1), target, num_cls=num_cls))
                    sep_loss += mask * masks_mod1 * (softmax_weighted_loss_bs(F.softmax(fuse_pred_t1ce, dim=1), target, num_cls=num_cls) + dice_loss_bs(F.softmax(fuse_pred_t1ce, dim=1), target, num_cls=num_cls))
                    sep_loss += mask * masks_mod2 * (softmax_weighted_loss_bs(F.softmax(fuse_pred_t1, dim=1), target, num_cls=num_cls) + dice_loss_bs(F.softmax(fuse_pred_t1, dim=1), target, num_cls=num_cls))
                    sep_loss += mask * masks_mod3 * (softmax_weighted_loss_bs(F.softmax(fuse_pred_t2, dim=1), target, num_cls=num_cls) + dice_loss_bs(F.softmax(fuse_pred_t2, dim=1), target, num_cls=num_cls))
                    fuse_loss = softmax_weighted_loss_bs(F.softmax(fuse_pred, dim=1), target, num_cls=num_cls) + dice_loss_bs(F.softmax(fuse_pred, dim=1), target, num_cls=num_cls)
                    return fuse_loss, sep_loss
                ...
                if self.mask_type == 'pdt':
                    ### Mod0-Flair-Path
                    sep_loss += masks_mod0 * (softmax_weighted_loss_bs(F.softmax(fuse_pred_flair, dim=1), target, num_cls=num_cls) + dice_loss_bs(F.softmax(fuse_pred_flair, dim=1), target, num_cls=num_cls))
                    ...
                    ### Mod1-T1c-Path
                    sep_loss += masks_mod1 * masks_mod1 * (softmax_weighted_loss_bs(F.softmax(fuse_pred_t1ce, dim=1), target, num_cls=num_cls) + dice_loss_bs(F.softmax(fuse_pred_t1ce, dim=1), target, num_cls=num_cls))
                    ...
                    ### Mod2-T1-Path
                    sep_loss += masks_mod2 * (softmax_weighted_loss_bs(F.softmax(fuse_pred_t1, dim=1), target, num_cls=num_cls) + dice_loss_bs(F.softmax(fuse_pred_t1, dim=1), target, num_cls=num_cls))
                    ...
                    ### Mod3-T2-Path
                    sep_loss += masks_mod3 * (softmax_weighted_loss_bs(F.softmax(fuse_pred_t2, dim=1), target, num_cls=num_cls) + dice_loss_bs(F.softmax(fuse_pred_t2, dim=1), target, num_cls=num_cls))
                ...
                if self.mask_type == 'pdt':
                    ...
                    sep_loss += masks_mod0 * (softmax_weighted_loss_bs(F.softmax(fuse_pred_flair, dim=1), target, num_cls=num_cls) + dice_loss_bs(F.softmax(fuse_pred_flair, dim=1), target, num_cls=num_cls))
                    sep_loss += masks_mod1 * (softmax_weighted_loss_bs(F.softmax(fuse_pred_t1ce, dim=1), target, num_cls=num_cls) + dice_loss_bs(F.softmax(fuse_pred_t1ce, dim=1), target, num_cls=num_cls))
                    sep_loss += masks_mod2 * (softmax_weighted_loss_bs(F.softmax(fuse_pred_t1, dim=1), target, num_cls=num_cls) + dice_loss_bs(F.softmax(fuse_pred_t1, dim=1), target, num_cls=num_cls))
                    sep_loss += masks_mod3 * (softmax_weighted_loss_bs(F.softmax(fuse_pred_t2, dim=1), target, num_cls=num_cls) + dice_loss_bs(F.softmax(fuse_pred_t2, dim=1), target, num_cls=num_cls))
                else: ### idt or idt_moddrop
                    # sep_loss += mask * masks_mod0 * (softmax_weighted_loss_bs(flair_pred, target, num_cls=num_cls) + dice_loss_bs(flair_pred, target, num_cls=num_cls))
