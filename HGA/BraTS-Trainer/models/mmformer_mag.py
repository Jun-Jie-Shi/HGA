...
from utils.criterions import temp_kl_loss, softmax_weighted_loss, dice_loss
...
class Model(nn.Module):
    def __init__(self, num_cls=4):
        ...
        self.multimodal_decode_conv = nn.Conv3d(transformer_basic_dims*num_modals, basic_dims*16, kernel_size=1, padding=0)
        ########### InterFormer
    ...
    def forward(self, x, mask, target=None, temp=1.0):
        ...
        multimodal_inter_token_x5 = multimodal_inter_token_x5.view(multimodal_inter_token_x5.size(0), patch_size, patch_size, patch_size, transformer_basic_dims*num_modals).permute(0, 4, 1, 2, 3).contiguous()
        modal_sum = mask[0].sum().cuda().float()
        fuse_avg = (flair_x5 + t1ce_x5 + t1_x5 + t2_x5) / modal_sum
        x1_avg = (flair_x1 + t1ce_x1 + t1_x1 + t2_x1) / modal_sum
        x2_avg = (flair_x2 + t1ce_x2 + t1_x2 + t2_x2) / modal_sum
        x3_avg = (flair_x3 + t1ce_x3 + t1_x3 + t2_x3) / modal_sum
        x4_avg = (flair_x4 + t1ce_x4 + t1_x4 + t2_x4) / modal_sum
        
        multimodal_inter_x5 = self.multimodal_decode_conv(multimodal_inter_token_x5)
        x5_inter = (multimodal_inter_x5 + fuse_avg) / 2.0

        fuse_pred, preds, de_f_avg = self.decoder_fuse(x1_avg, x2_avg, x3_avg, x4_avg, x5_inter)
        # fuse_pred, preds, de_f_avg = self.decoder_fuse(x1_avg, x2_avg, x3_avg, x4_avg, x5_avg, mask)
        ########### InterFormer
        
        if self.is_training:
            kl_loss = torch.zeros(4).cuda().float()
            sep_cross_loss = torch.zeros(4).cuda().float()
            sep_dice_loss = torch.zeros(4).cuda().float()
            prm_cross_loss = torch.zeros(1).cuda().float()
            prm_dice_loss = torch.zeros(1).cuda().float()
            proto_loss = torch.zeros(4).cuda().float()
            fkl_loss = torch.zeros(4).cuda().float()
            if mask[0,0]:

                fuse_pred_flair, preds_flair, de_f_flair = self.decoder_fuse(flair_x1, flair_x2, flair_x3, flair_x4, flair_x5, mask)
                fkl_loss[0] += torch.nn.MSELoss()(flair_x5, fuse_avg)

            if mask[0,1]:

                fuse_pred_t1ce, preds_t1ce, de_f_t1ce = self.decoder_fuse(t1ce_x1, t1ce_x2, t1ce_x3, t1ce_x4, t1ce_x5, mask)
                fkl_loss[1] += torch.nn.MSELoss()(t1ce_x5, fuse_avg)

            if mask[0,2]:

                fuse_pred_t1, preds_t1, de_f_t1 = self.decoder_fuse(t1_x1, t1_x2, t1_x3, t1_x4, t1_x5, mask)
                fkl_loss[2] += torch.nn.MSELoss()(t1_x5, fuse_avg)

            if mask[0,3]:

                fuse_pred_t2, preds_t2, de_f_t2 = self.decoder_fuse(t2_x1, t2_x2, t2_x3, t2_x4, t2_x5, mask)
                fkl_loss[3] += torch.nn.MSELoss()(t2_x5, fuse_avg)

            if mask[0,0]:
                # sep_cross_loss[0] = softmax_weighted_loss(flair_pred, target, num_cls=num_cls)
                # sep_dice_loss[0] = dice_loss(flair_pred, target, num_cls=num_cls)
                kl_loss[0] += temp_kl_loss(fuse_pred_flair, fuse_pred.detach(), target, num_cls=num_cls, temp=temp)
            if mask[0,1]:
                # sep_cross_loss[1] = softmax_weighted_loss(t1ce_pred, target, num_cls=num_cls)
                # sep_dice_loss[1] = dice_loss(t1ce_pred, target, num_cls=num_cls)
                kl_loss[1] += temp_kl_loss(fuse_pred_t1ce, fuse_pred.detach(), target, num_cls=num_cls, temp=temp)
            if mask[0,2]:
                # sep_cross_loss[2] = softmax_weighted_loss(t1_pred, target, num_cls=num_cls)
                # sep_dice_loss[2] = dice_loss(t1_pred, target, num_cls=num_cls)
                kl_loss[2] += temp_kl_loss(fuse_pred_t1, fuse_pred.detach(), target, num_cls=num_cls, temp=temp)
            if mask[0,3]:
                # sep_cross_loss[3] = softmax_weighted_loss(t2_pred, target, num_cls=num_cls)
                # sep_dice_loss[3] = dice_loss(t2_pred, target, num_cls=num_cls)
            ...
            return F.softmax(fuse_pred, dim=1), (prm_cross_loss, prm_dice_loss), (sep_cross_loss, sep_dice_loss), kl_loss, proto_loss, fkl_loss
        return F.softmax(fuse_pred, dim=1)
        #     return fuse_pred, (flair_pred, t1ce_pred, t1_pred, t2_pred), preds
        # return fuse_pred
        # return self.decoder_sep(flair_x1, flair_x2, flair_x3, flair_x4, flair_x5)