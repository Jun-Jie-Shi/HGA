...
class Generic_nnunet(nn.Module):
    DEFAULT_BATCH_SIZE_3D = 2
    DEFAULT_PATCH_SIZE_3D = (64, 192, 160)
    SPACING_FACTOR_BETWEEN_STAGES = 2
    BASE_NUM_FEATURES_3D = 30
    ...
    def __init__(self, modality_num=4, base_num_features=48, num_classes=4, num_pool=4, num_conv_per_stage=2,
                 ...
                 nonlin=nn.LeakyReLU, nonlin_kwargs={'negative_slope': 1e-2, 'inplace': True}, deep_supervision=True, dropout_in_localization=False,
                 final_nonlin=softmax_helper, weightInitializer=InitWeights_He(1e-2), pool_op_kernel_sizes=None,
                 conv_kernel_sizes=None,
                 upscale_logits=True, convolutional_pooling=True, convolutional_upsampling=True,
                 max_num_features=None, basic_block=ConvDropoutNormNonlin,
                 seg_output_use_bias=False):
        """
        basically more flexible than v1, architecture is the same
        ...
        Questions? -> f.isensee@dkfz.de
        """
        super(Generic_nnunet, self).__init__()
        self.inference_apply_nonlin = lambda x: x  # softmax_helper
        self._gaussian_3d = self._patch_size_for_gaussian_3d = None
        self._gaussian_2d = self._patch_size_for_gaussian_2d = None
        ...
        # self.modality_specific_models = Generic_UNet(modality_num, base_num_features, num_classes, num_pool, num_conv_per_stage, 
    ...
    def forward(self, x, mask):
        ...
        if self._deep_supervision and self.is_training:
            ...
            return out, modality_features, tuple(final_outputs)
        else:
            # return output
            return nn.Softmax(dim=1)(out_mul)