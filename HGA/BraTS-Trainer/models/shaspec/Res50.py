...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, norm_cfg, activation_cfg, stride=(1, 1, 1), downsample=None, weight_std=False):
        ...
        self.conv2 = conv3x3x3(inplanes, planes, kernel_size=3, stride=(1, 1, 1), padding=1, bias=False, weight_std=weight_std)
...
class BottleneckTiny(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, norm_cfg, activation_cfg, stride=(1, 1, 1), downsample=None, weight_std=False):
        super(BottleneckTiny, self).__init__()
        # self.conv1 = conv3x3x3(inplanes, planes, kernel_size=1, bias=False, weight_std=weight_std)
        # self.norm1 = Norm_layer(norm_cfg, planes)
        self.conv2 = conv3x3x3(inplanes, planes, kernel_size=3, stride=stride, padding=1, bias=False, weight_std=weight_std)
        self.norm2 = Norm_layer(norm_cfg, planes)
...
class ResNet(nn.Module):
    ...
    arch_settings = {
        10: (BottleneckTiny, (1, 1, 1, 1)),
        # 10: (BottleneckTiny, (2, 2, 2, 2)),
    ...
    def __init__(self,
        ...
        self.conv1 = conv3x3x3(in_channels, 64, kernel_size=7, stride=(1, 1, 1), padding=3, bias=False, weight_std=weight_std)
        self.norm1 = Norm_layer(norm_cfg, 64)
        self.nonlin = Activation_layer(activation_cfg, inplace=True)
        self.maxpool = nn.MaxPool3d(kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=1)
        self.layer1 = self._make_layer(block, 64, layers[0], shortcut_type, stride=(1, 1, 1), norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std)
