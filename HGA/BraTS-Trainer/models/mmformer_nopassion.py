...
class Model(nn.Module):
    ...
    def forward(self, x, mask, target=None, temp=1.0, only_multi=None, only_uni=None):
        B = x.size(0)
        device = x.device
        ...
        if self.is_training:
            ...
            unimodal_loss = torch.zeros(B,4).float().to(device)
            # fuse_pred_flair = F.softmax(fuse_pred_flair, dim=1)
            # fuse_pred_t1ce = F.softmax(fuse_pred_t1ce, dim=1)
            # fuse_pred_t1 = F.softmax(fuse_pred_t1, dim=1)
            # fuse_pred_t2 = F.softmax(fuse_pred_t2, dim=1)

            if self.mask_type == 'pdt':
                ### Mod0-Flair-Path
                unimodal_loss += masks_mod0 * (softmax_weighted_loss_bs(flair_pred, target, num_cls=num_cls) + dice_loss_bs(flair_pred, target, num_cls=num_cls))
                ### Mod1-T1c-Path
                unimodal_loss += masks_mod1 * (softmax_weighted_loss_bs(t1ce_pred, target, num_cls=num_cls) + dice_loss_bs(t1ce_pred, target, num_cls=num_cls))
                ### Mod2-T1-Path
                unimodal_loss += masks_mod2 * (softmax_weighted_loss_bs(t1_pred, target, num_cls=num_cls) + dice_loss_bs(t1_pred, target, num_cls=num_cls))
                ### Mod3-T2-Path
                unimodal_loss += masks_mod3 * (softmax_weighted_loss_bs(t2_pred, target, num_cls=num_cls) + dice_loss_bs(t2_pred, target, num_cls=num_cls))
            else: ### idt or idt_moddrop
                ### Mod0-Flair-Path
                if mask[0,0]:
                    unimodal_loss += mask * masks_mod0 * (softmax_weighted_loss_bs(flair_pred, target, num_cls=num_cls) + dice_loss_bs(flair_pred, target, num_cls=num_cls))
                ### Mod1-T1c-Path
                if mask[0,1]:
                    unimodal_loss += mask * masks_mod1 * (softmax_weighted_loss_bs(t1ce_pred, target, num_cls=num_cls) + dice_loss_bs(t1ce_pred, target, num_cls=num_cls))
                ### Mod2-T1-Path
                if mask[0,2]:
                    unimodal_loss += mask * masks_mod2 * (softmax_weighted_loss_bs(t1_pred, target, num_cls=num_cls) + dice_loss_bs(t1_pred, target, num_cls=num_cls))
                ### Mod3-T2-Path
                if mask[0,3]:
                    unimodal_loss += mask * masks_mod3 * (softmax_weighted_loss_bs(t2_pred, target, num_cls=num_cls) + dice_loss_bs(t2_pred, target, num_cls=num_cls))
                # ### Mod0-Flair-Path
                # unimodal_loss += mask * masks_mod0 * (softmax_weighted_loss_bs(fuse_pred_flair, target, num_cls=num_cls))
                # ### Mod1-T1c-Path
                # unimodal_loss += mask * masks_mod1 * (softmax_weighted_loss_bs(fuse_pred_t1ce, target, num_cls=num_cls))
                # ### Mod2-T1-Path
                # unimodal_loss += mask * masks_mod2 * (softmax_weighted_loss_bs(fuse_pred_t1, target, num_cls=num_cls))
                # ### Mod3-T2-Path
                # unimodal_loss += mask * masks_mod3 * (softmax_weighted_loss_bs(fuse_pred_t2, target, num_cls=num_cls))
            if only_uni:
                return unimodal_loss
            else:
                return F.softmax(fuse_pred, dim=1), unimodal_loss, preds
