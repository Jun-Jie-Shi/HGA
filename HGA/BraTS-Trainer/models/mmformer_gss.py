        ...
        if self.is_training:
            ...
            if self.use_passion:                                
                ...
                if self.mask_type == 'pdt':
                    ...
                    sep_loss += mask * masks_mod0 * (softmax_weighted_loss_bs(flair_pred, target, num_cls=num_cls) + dice_loss_bs(flair_pred, target, num_cls=num_cls)+temp_kl_loss_bs(flair_pred, teacher_pred.detach(), target, num_cls=num_cls))
                    sep_loss += mask * masks_mod1 * (softmax_weighted_loss_bs(t1ce_pred, target, num_cls=num_cls) + dice_loss_bs(t1ce_pred, target, num_cls=num_cls)+temp_kl_loss_bs(t1ce_pred, teacher_pred.detach(), target, num_cls=num_cls))
                    sep_loss += mask * masks_mod2 * (softmax_weighted_loss_bs(t1_pred, target, num_cls=num_cls) + dice_loss_bs(t1_pred, target, num_cls=num_cls)+temp_kl_loss_bs(t1_pred, teacher_pred.detach(), target, num_cls=num_cls))
                    sep_loss += mask * masks_mod3 * (softmax_weighted_loss_bs(t2_pred, target, num_cls=num_cls) + dice_loss_bs(t2_pred, target, num_cls=num_cls)+temp_kl_loss_bs(t2_pred, teacher_pred.detach(), target, num_cls=num_cls))
