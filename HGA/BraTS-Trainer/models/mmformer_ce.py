...
class Decoder_fuse(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, x5, mask=None):
        ...
        pred3 = self.softmax(self.seg_d3(de_x4_f))
        ...
        pred2 = self.softmax(self.seg_d2(de_x3_f))
        ...
        pred1 = self.softmax(self.seg_d1(de_x2_f))
        ...
        return pred, (self.up2(pred1), self.up4(pred2), self.up8(pred3), self.up16(pred4)), (de_x1_f, de_x2_f, de_x3_f, de_x4_f, de_x5_f)
    ...
    def forward(self, x, mask, target=None, temp=1.0):
        ...
        if self.is_training:
            ...
            if self.use_passion:                                
                ...
                weight_prm = 1.0
                for prm_pred in preds:
                    weight_prm /= 2.0
                    prm_loss += weight_prm * softmax_weighted_loss_bs(prm_pred, target, num_cls=num_cls) \
                    + weight_prm * dice_loss_bs(prm_pred, target, num_cls=num_cls)
                ...
                if self.mask_type == 'pdt':
                    ...
                    kl_loss += masks_mod0 * softmax_weighted_loss_bs(fuse_pred_flair, target, num_cls=num_cls, up_op=up_op)
                    weight_prm = 1.0
                    for prm_pred, prm_pred_flair, up_op in zip(preds, preds_flair, self.up_ops):
                        weight_prm /= 2.0
                        kl_loss += masks_mod0 * weight_prm * softmax_weighted_loss_bs(prm_pred_flair, target, num_cls=num_cls, up_op=up_op)
                    ...
                    kl_loss += masks_mod1 * softmax_weighted_loss_bs(fuse_pred_t1ce, target, num_cls=num_cls, up_op=up_op)
                    weight_prm = 1.0
                    for prm_pred, prm_pred_t1ce, up_op in zip(preds, preds_t1ce, self.up_ops):
                        weight_prm /= 2.0
                        kl_loss += masks_mod1 * weight_prm * softmax_weighted_loss_bs(prm_pred_t1ce, target, num_cls=num_cls, up_op=up_op)
                    ...
                    kl_loss += masks_mod2 * softmax_weighted_loss_bs(fuse_pred_t1, target, num_cls=num_cls, up_op=up_op)
                    weight_prm = 1.0
                    for prm_pred, prm_pred_t1, up_op in zip(preds, preds_t1, self.up_ops):
                        weight_prm /= 2.0
                        kl_loss += masks_mod2 * weight_prm * softmax_weighted_loss_bs(prm_pred_t1, target, num_cls=num_cls, up_op=up_op)
                    ...
                    kl_loss += masks_mod3 * softmax_weighted_loss_bs(fuse_pred_t2, target, num_cls=num_cls, up_op=up_op)
                    weight_prm = 1.0
                    for prm_pred, prm_pred_t2, up_op in zip(preds, preds_t2, self.up_ops):
                        weight_prm /= 2.0
                        kl_loss += masks_mod3 * weight_prm * softmax_weighted_loss_bs(prm_pred_t2, target, num_cls=num_cls, up_op=up_op)
                    ...
                    kl_loss += mask * masks_mod0 * softmax_weighted_loss_bs(fuse_pred_flair, target, num_cls=num_cls)
                    weight_prm = 1.0
                    for prm_pred, prm_pred_flair in zip(preds, preds_flair):
                        weight_prm /= 2.0
                        kl_loss += mask * masks_mod0 * weight_prm * softmax_weighted_loss_bs(prm_pred_flair, target, num_cls=num_cls)
                    ...
                    kl_loss += mask * masks_mod1 * softmax_weighted_loss_bs(fuse_pred_t1ce, target, num_cls=num_cls)
                    weight_prm = 1.0
                    for prm_pred, prm_pred_t1ce in zip(preds, preds_t1ce):
                        weight_prm /= 2.0
                        kl_loss += mask * masks_mod1 * weight_prm * softmax_weighted_loss_bs(prm_pred_t1ce, target, num_cls=num_cls)
                    ...
                    kl_loss += mask * masks_mod2 * softmax_weighted_loss_bs(fuse_pred_t1, target, num_cls=num_cls)
                    weight_prm = 1.0
                    for prm_pred, prm_pred_t1 in zip(preds, preds_t1):
                        weight_prm /= 2.0
                        kl_loss += mask * masks_mod2 * weight_prm * softmax_weighted_loss_bs(prm_pred_t1, target, num_cls=num_cls)
                    ...
                    kl_loss += mask * masks_mod3 * softmax_weighted_loss_bs(fuse_pred_t2, target, num_cls=num_cls)
                    weight_prm = 1.0
                    for prm_pred, prm_pred_t2 in zip(preds, preds_t2):
                        weight_prm /= 2.0
                        kl_loss += mask * masks_mod3 * weight_prm * softmax_weighted_loss_bs(prm_pred_t2, target, num_cls=num_cls)

                return fuse_pred, prm_loss, sep_loss, kl_loss, proto_loss, dist
                ...
                return fuse_pred, prm_loss, sep_loss
