...
class PrototypeTable(nn.Module):
    def __init__(self, num_cls, feature_dim):
        ...
        all_combinations = [[False, False, False, True], [False, True, False, False], [False, False, True, False], [True, False, False, False],
         [False, True, False, True], [False, True, True, False], [True, False, True, False], [False, False, True, True], [True, False, False, True], [True, True, False, False],
         [True, True, True, False], [True, False, True, True], [True, True, False, True], [False, True, True, True],
         [True, True, True, True]]
        self.all_combinations = all_combinations

        self.prototype_tables = {
            str(tuple(combination)): torch.zeros(num_cls, feature_dim).cuda().detach()
            for combination in all_combinations
        }
        self.prototypes = torch.zeros(num_cls, feature_dim).cuda().detach()
        self.tmp = torch.zeros(1, feature_dim).cuda().detach()

    def forward(self, x, modality_mask, target=None, available_mask=None, proto_update=False):
        key = str(tuple(modality_mask.squeeze().tolist()))
        if target is not None:
            ...
            for i in range(num_cls):
                ...
                if (torch.sum(targeti,dim=(-3,-2,-1))>0).all():
                    proto_batch =  (torch.sum(x * targeti[:,None],dim=(-3,-2,-1))/(torch.sum(targeti[:,None],dim=(-3,-2,-1)) + 1e-5)).squeeze(0)
                    if torch.equal(self.tmp, self.prototype_tables[key][i]):
                        self.prototype_tables[key][i] = proto_batch.detach()
                        if proto_update:
                            self.prototypes[i] = proto_batch.detach()
                    if proto_update:
                        prototypes_i = 0.2 * self.prototypes[i] + 0.8 * proto_batch
                        # prototypes_i = proto_batch
                    else:
                        prototypes_i = 0.2 * self.prototype_tables[key][i] + 0.8 * proto_batch
                        # prototypes_i = proto_batch

                    distance = -torch.sqrt(torch.sum((x - prototypes_i.unsqueeze(0)[:,:,None,None,None])**2, dim=1))
                    distances.append(distance.unsqueeze(1))
                    gt.append(targeti[:,None])

                    self.prototype_tables[key][i] = 0.2 * self.prototype_tables[key][i] + 0.8 * proto_batch.detach()
                    # self.prototype_tables[key][i] = proto_batch.detach()
                    if proto_update:
                        self.prototypes[i] = 0.2 * self.prototypes[i] + 0.8 * proto_batch.detach()
                # gt.append(targeti[:,None])
        # distances = []
        # if proto_update:
        #     # prototypes = self.prototypes
        #     prototypes = self.prototype_tables[key]
        # else:
        #     prototypes = self.prototype_tables[key]
        # for j, prototype in enumerate(prototypes):
        #     targetj = target[:, j, :, :, :]
        #     if (torch.sum(targetj,dim=(-3,-2,-1))>0).all():
        #         distance = -torch.sqrt(torch.sum((x - prototype.unsqueeze(0)[:,:,None,None,None])**2, dim=1))
        #         distances.append(distance.unsqueeze(1))
        #         gt.append(targetj[:,None])
        # for prototype in prototypes:
        #     distance = -torch.sqrt(torch.sum((x - prototype.unsqueeze(0)[:,:,None,None,None])**2, dim=1))
        #     distances.append(distance.unsqueeze(1))

        softmax_s = torch.nn.Softmax(dim=1)(torch.cat(distances, dim=1))
        if target is not None:
            ...
            return softmax_s, proto_loss, dist
        return softmax_s
            # key_t = str(tuple(availabel_mask))
            # prototypes_t = self.prototype_tables[key_t]
            # distances_t = []
            # for prototype_t in prototypes_t:
            #     distance_t = -torch.sqrt(torch.sum((x - prototype_t[:,:,None,None,None])**2, dim=1))
            #     distances_t.append(distance_t.unsqueeze(1))
            # softmax_t = torch.nn.Softmax(dim=1)(torch.cat(distances_t, dim=1))
            # proto_distri_t = torch.sum(softmax_t*gt, dim=1)
            # proto_loss_t = torch.mean(-torch.log(torch.clamp(proto_distri_t, min=0.005, max=1)))
            # dist_t = torch.mean(proto_distri_t)
...
class Model(nn.Module):
    def __init__(self, num_cls=4):
        ...
        self.proto_seg = PrototypeTable(num_cls=num_cls, feature_dim=basic_dims)
    ...
    def forward(self, x, mask, target=None, temp=1.0):
        ...
        if self.is_training:
            softmax_fuse, proto_fuse_loss, dist_fuse = self.proto_seg(de_f_avg[0], mask, target=target, proto_update=True)
        # else:
        #     softmax_fuse = self.proto_seg(de_f_avg[0], mask)
        ########### InterFormer end
        if self.is_training:
            if self.mask_type == 'pdt':
            ...
            if self.use_passion:                                
                ...
                if self.mask_type == 'pdt':
                    ...
                    ### Mod0-Flair-Path
                    softmax_mod0, proto_loss_mod0, dist_mod0 = self.proto_seg(de_f_flair[0], masks_mod0, target=target, available_mask=mask)
                    ...
                    ### Mod1-T1c-Path
                    softmax_mod1, proto_loss_mod1, dist_mod1 = self.proto_seg(de_f_t1ce[0], masks_mod1, target=target, available_mask=mask)
                    ...
                    ### Mod2-T1-Path
                    softmax_mod2, proto_loss_mod2, dist_mod2 = self.proto_seg(de_f_t1[0], masks_mod2, target=target, available_mask=mask)
                    ...
                    ### Mod3-T2-Path
                    softmax_mod3, proto_loss_mod3, dist_mod3 = self.proto_seg(de_f_t2[0], masks_mod3, target=target, available_mask=mask)
                ...
                return F.softmax(fuse_pred, dim=1), prm_loss, sep_loss, kl_loss, proto_loss, dist, proto_fuse_loss
                # return softmax_fuse, prm_loss, sep_loss, kl_loss, proto_loss, dist
