...
from models.shaspec import DualNet_SS
# import torch.optim
...
def main():
    ...
    elif args.model == 'shaspec':
        model = DualNet_SS.DualNet_SS_Mag(args=args, norm_cfg='IN', activation_cfg='LeakyReLU',
                        num_classes=num_cls, weight_std=True, self_att=False, cross_att=False)
    ...
    if args.use_passion:
        ...
        for epoch in range(args.num_epochs):
            ...
            epoch_shared_losses = torch.zeros(1).cpu().float()
            epoch_spec_losses = torch.zeros(1).cpu().float()
            ...
            for i in range(iter_per_epoch):
                ...
                preds, (logits0, logits1, logits2, logits3), (fep, fe0, fe1, fe2, fe3), shared_info, spec_info, mode_split = model(x, mask)
                # losses = []
                # losses_summary = []
                # for out_bs_ava in out_bs:
                #     losses.append(criterions.DiceCoef(out_bs_ava, target, num_cls=num_cls))
                # cnt = 0
                term_seg_Dice = criterions.DiceCoef(preds, target, num_cls=num_cls)
                term_seg_BCE = criterions.softmax_weighted_loss(torch.nn.Softmax(dim=1)(preds), target, num_cls=num_cls)

                N, C, D, W, H = x.shape

                flair_pred = torch.nn.Softmax(dim=1)(logits0)
                t1ce_pred = torch.nn.Softmax(dim=1)(logits1)
                t1_pred = torch.nn.Softmax(dim=1)(logits2)
                t2_pred = torch.nn.Softmax(dim=1)(logits3)

                if mask[0,0]:
                    if mask[0,1]|mask[0,2]|mask[0,3]:
                        teacher_pred = torch.zeros_like(flair_pred).cuda().float()
                        all_one = torch.ones_like(flair_pred).cuda().float()
                        pred_mask_l = flair_pred > 0.65
                        pred_mask_m = (((t1ce_pred > 0.75)*mask[0,1].float() + (t1_pred > 0.75)*mask[0,2].float() + (t2_pred > 0.75)*mask[0,3].float()) == (mask[0,1].float()+mask[0,2].float()+mask[0,3].float()))*(flair_pred <= 0.65)
                        pred_mask_s = (all_one - pred_mask_l.cuda().float() - pred_mask_m.cuda().float()) == all_one

                        pred_s = torch.zeros_like(flair_pred).cuda().float()
                        teacher_pred = pred_mask_l*flair_pred + pred_mask_m*(t1ce_pred*(mask[0,1].float())+t1_pred*(mask[0,2].float())+t2_pred*(mask[0,3].float()))/(mask[0,1].float()+mask[0,2].float()+mask[0,3].float())
                        pred_flair = flair_pred
                        pred_t1ce = t1ce_pred if mask[0,1] else torch.ones_like(flair_pred).cuda().float()
                        pred_t1 = t1_pred if mask[0,2] else torch.ones_like(flair_pred).cuda().float()
                        pred_t2 = t2_pred if mask[0,3] else torch.ones_like(flair_pred).cuda().float()
                        for j in range(4):
                            pred_s[0,j,:] = torch.amin(torch.cat((pred_flair[0,j,:], pred_t1ce[0,j,:], pred_t1[0,j,:], pred_t2[0,j,:]), dim=1), dim=1)

                        teacher_pred += pred_mask_s*pred_s
                    else:
                        teacher_pred = flair_pred
                else:
                    pred_mask_m = ((t1ce_pred > 0.75)*mask[0,1].float() + (t1_pred > 0.75)*mask[0,2].float() + (t2_pred > 0.75)*mask[0,3].float()) == (mask[0,1].float()+mask[0,2].float()+mask[0,3].float())
                    all_one = torch.ones_like(pred_mask_m).cuda().float()
                    pred_mask_s = (all_one - pred_mask_m.cuda().float()) == all_one
                    teacher_pred = pred_mask_m*((t1ce_pred*(mask[0,1].float())+t1_pred*(mask[0,2].float())+t2_pred*(mask[0,3].float()))/(mask[0,1].float()+mask[0,2].float()+mask[0,3].float()))
                    pred_t1ce = t1ce_pred if mask[0,1] else torch.ones_like(pred_mask_m).cuda().float()
                    pred_t1 = t1_pred if mask[0,2] else torch.ones_like(pred_mask_m).cuda().float()
                    pred_t2 = t2_pred if mask[0,3] else torch.ones_like(pred_mask_m).cuda().float()
                    pred_s = torch.zeros_like(pred_mask_m).cuda().float()
                    for j in range(4):
                        pred_s[0,j,:] = torch.amin(torch.cat((pred_t1ce[0,j,:], pred_t1[0,j,:], pred_t2[0,j,:]), dim=1), dim=1)
                    teacher_pred += pred_mask_s*pred_s
                ...
                # torch.sum(kl_loss_m_bs*mask, dim=0)
                term_shared = torch.nn.L1Loss()(shared_info[0:N * C + 1:C], shared_info[1:N * C + 1:C]) + \
                            torch.nn.L1Loss()(shared_info[1:N * C + 1:C], shared_info[2:N * C + 1:C]) + \
                            torch.nn.L1Loss()(shared_info[2:N * C + 1:C], shared_info[3:N * C + 1:C]) + \
                            torch.nn.L1Loss()(shared_info[3:N * C + 1:C], shared_info[0:N * C + 1:C])

                spec_labels = torch.zeros(N * C, dtype=torch.long).cuda()
                spec_labels[0:N * C + 1:C] = 0
                spec_labels[1:N * C + 1:C] = 1
                spec_labels[2:N * C + 1:C] = 2
                spec_labels[3:N * C + 1:C] = 3
                term_spec = torch.nn.CrossEntropyLoss()(spec_info, spec_labels)
                loss = term_seg_Dice + term_seg_BCE + 0.1 * term_shared + 0.02 * term_spec + 0.5 * term_kl
                ...
                epoch_fuse_losses += (term_seg_Dice/iter_per_epoch).detach().cpu()
                epoch_shared_losses += (term_shared/iter_per_epoch).detach().cpu()
                epoch_spec_losses += (term_spec/iter_per_epoch).detach().cpu()
                ...
                msg += 'dice_loss:{:.4f}, '.format(term_seg_Dice.item())
                msg += 'wce_loss:{:.4f}, '.format(term_seg_BCE.item())
                msg += 'shared_loss:{:.4f}, spec_loss:{:.4f}, '.format(term_shared.item(), term_spec.item())
                ...
                # msg += 'fkllist:[{:.4f},{:.4f},{:.4f},{:.4f}] '.format(fkl0.item(), fkl1.item(), fkl2.item(), fkl3.item())
            ...
            writer.add_scalar('epoch_shared_losses', epoch_shared_losses.item(), global_step=(epoch+1))
            writer.add_scalar('epoch_spec_losses', epoch_spec_losses.item(), global_step=(epoch+1))
