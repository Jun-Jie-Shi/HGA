...
def purity_score_bs(feature_s, target, num_cls=4):
    target = target.float()
    eps = 1e-5
    s = []
    gt = []
...
class DRe(nn.Module):
    ...
    def dre_backward(self, objectives, outputs, target, active_tasks=None):
        ...
        if len(active_indices) == 4:
            ...
            if self.all_modal_account % 15 == 0:
    ...
    def re_learning(self, objectives, outputs, target, phi_tilde=None, active_tasks=None):
        # --- 更新存在的任务 ---
        device = self.task_weights.device
        ...
        for i in range(self.n_tasks):
            ...
            purity_score_d = purity_score_bs(outputs[i][:1], target[:1])
            purity_score_v = purity_score_bs(outputs[i][1:], target[1:])
            # purity_d[i] = purity_score[0]
            # purity_v[i] = purity_score[1]
            # g_k[i] = torch.abs(purity_score[0] - purity_score[1])
            g_k[i] = torch.abs(purity_score_d- purity_score_v)
        ...
        if phi_tilde is None:
            ...
            param_tilde = self._flatten_grad(phi_tilde, shapes)
        self.reinit_update(shapes, has_grads, param, param_tilde, alpha_k)
        logging.info('Re-Learning Alpha:[{:.4f},{:.4f},{:.4f},{:.4f}]'.format(alpha_k[0].item(),alpha_k[1].item(),alpha_k[2].item(),alpha_k[3].item()))
        self.rebalanced = True
        return
    ...
    def reinit_update(self, shapes, has_grads, param, param_tilde, alpha_k):
        ...
        # param[shared] -= torch.mean(alpha_k) * (param[shared] - param_tilde[shared])
