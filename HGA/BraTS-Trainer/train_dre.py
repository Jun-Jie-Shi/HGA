...
def main():
    ...
    if not args.use_passion:
        ...
        # device = model.device
        dre_adam = DRe(optimizer, n_tasks=4, writer=writer)
    else:
    ...
    if not args.only_eval:
        ...
        #     exit(0)
        train_setd = Brats_loadall_metaval_nii_idt(transforms=args.train_transforms, root=args.datasetPath, mask_value=14, num_cls=num_cls, train_file=train_file)
    ...
    test_loader = MultiEpochsDataLoader(
        ...
        num_workers=0,
        pin_memory=True)
    d_loader = MultiEpochsDataLoader(
        dataset=train_setd,
        batch_size=1,
        num_workers=8,
        pin_memory=True,
        ...
        if args.use_passion:
            ...
            for epoch in range(args.num_epochs):
                ...
                for i in range(iter_per_epoch):
                    ...
                    kl0 = criterions.temp_kl_loss_bs(out_bs[0], out_bs[4].detach(), target, num_cls=num_cls, temp=temp)
                    kl1 = criterions.temp_kl_loss_bs(out_bs[1], out_bs[4].detach(), target, num_cls=num_cls, temp=temp)
                    kl2 = criterions.temp_kl_loss_bs(out_bs[2], out_bs[4].detach(), target, num_cls=num_cls, temp=temp)
                    kl3 = criterions.temp_kl_loss_bs(out_bs[3], out_bs[4].detach(), target, num_cls=num_cls, temp=temp)
                    ...
                    loss_reg = 0.5 * term_kl + 0.1 * term_proto + term_seg_dice + torch.sum(torch.stack(losses, dim=0))
            ...
            for epoch in range(args.num_epochs):
                ...
                b = time.time()
                for i in range(iter_per_epoch):
                    if not dre_adam.check_relearning():
                        try:
                            data_d = next(d_iter)
                        except:
                            d_iter = iter(d_loader)
                        ...
                        try:
                            ...
                            v_iter = iter(v_loader)
                            data_v = next(v_iter)
                        x_d, target_d, mask_d, name_d = data_d
                        x_d = x_d.cuda(non_blocking=True)
                        target_d = target_d.cuda(non_blocking=True)
                        mask_d = mask_d.cuda(non_blocking=True)

                        x_v, target_v, mask_v, name_v = data_v
                        x_v = x_v.cuda(non_blocking=True)
                        target_v = target_v.cuda(non_blocking=True)
                        mask_v = mask_v.cuda(non_blocking=True)

                        x_dv = torch.cat([x_d,x_v], dim=0)
                        target_dv = torch.cat([target_d,target_v], dim=0)
                        mask_dv = torch.cat([mask_d,mask_v], dim=0)

                        model.module.is_training = True
                        out_dv_bs, de_fe_dv_bs = model(x_dv, mask_dv, target=target_dv)
                        objs_dv=[]
                        for modc in range(4):
                            objs_dv.append(criterions.DiceCoef(out_dv_bs[modc], target_dv, num_cls=num_cls))
                        dre_adam.re_learning(objs_dv, out_dv_bs, target_dv)
                        dre_adam.zero_grad()
                    step = (i+1) + epoch*iter_per_epoch
                    ###Data load
                    try:
                    ...
                    dre_adam.zero_grad()
                    dre_adam.dre_backward(losses_summary, out_bs, target)
                    dre_adam.step()
