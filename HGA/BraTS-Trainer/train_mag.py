                ...
                for i in range(iter_per_epoch):
                    ...
                    for modc in range(4):
                        if mask[0,modc]:
                            ...
                            losses_summary.append(torch.nn.MSELoss()(de_fe_bs[modc], de_fe_bs[4])+criterions.temp_kl_loss(torch.nn.Softmax(dim=1)(out_bs[modc]), torch.nn.Softmax(dim=1)(out_bs[4]).detach(), target, num_cls=num_cls, temp=temp)+criterions.softmax_weighted_loss(torch.nn.Softmax(dim=1)(out_bs[modc]), target, num_cls=num_cls)+ criterions.dice_loss(torch.nn.Softmax(dim=1)(out_bs[modc]), target, num_cls=num_cls))
                            # cnt += 1
                        else:
                            losses_summary.append(torch.tensor(0).cuda().float())
