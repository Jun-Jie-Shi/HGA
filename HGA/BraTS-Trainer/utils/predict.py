...
# def compute_foreground_ece_3d(pred_probs, labels, num_bins=20, class_names=None):
    
#     # === Step 1: Collect ALL foreground pixels from all samples ===
#     all_fg_probs = []  # List of (num_fg_pixels_in_sample, C)
#     all_fg_labels = [] # List of (num_fg_pixels_in_sample,)
    
#     total_foreground_count = len(labels)
...
#     total_foreground = len(fg_labels)

#     for c_idx, class_label in enumerate([1, 2, 3]):  # WT TC ET
#         # One-vs-Rest: probability of this foreground class
#         confidences = fg_probs[:, class_label]  # (NumForeground,)
#         binary_labels = (fg_labels == class_label).astype(int)  # 1 if true label == class_label
...
#         # Normalize by total *foreground* pixels (not all pixels)
#         if total_foreground > 0:
#             ece /= total_foreground
#         else:
#             ece = 0.0

#         ece_foreground.append(ece)

#     # Macro-average over foreground classes
#     macro_ece_fg = np.mean(ece_foreground)

#     return ece_foreground, macro_ece_fg

def test_ece_softmax(
        test_loader,
        model,
    ...
    for c_idx, class_label in enumerate([1, 2, 3]):  # ET, TC, WT
