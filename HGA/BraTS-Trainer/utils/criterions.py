...
def prototype_kl_loss_4(feature_s, feature_t, target, logit_s, logit_t, num_cls=5, temp=1.0, up_op=None):
    target = target.float()
    eps = 1e-5
    N = len(feature_s.size()) - 2
    s = []
    ...
    kl_loss = torch.mean(torch.sqrt((sim_map_s-sim_map_t)**2))
...
def prototype_passion_loss_bs(feature_s, feature_t, target, logit_s=None, logit_t=None, num_cls=5, temp=1.0, up_op=None):
    ...
    # sim_map_t_gt = torch.sum(sim_map_t*gt, dim=1, keepdim=True)
