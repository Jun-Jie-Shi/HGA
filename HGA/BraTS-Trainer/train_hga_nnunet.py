...
def main():
    ...
    if args.model == 'nnunet':
        model = nnunet_inc.Generic_nnunet(num_classes=num_cls)
        ...
        if args.use_passion:
            ...
            for epoch in range(args.num_epochs):
                ...
                for i in range(iter_per_epoch):
                    ...
                    out_bs, de_fe_bs, prm_preds = model(x, mask)
                    losses = []
                    losses_summary = []
                    kl_losses = []
                    ...
                    for modc in range(4):
                        if mask[0,modc]:
                            ...
                            losses.append(imb_beta[modc]*(criterions.DiceCoef(out_bs[modc], target, num_cls=num_cls) + criterions.softmax_weighted_loss(torch.nn.Softmax(dim=1)(out_bs[modc]), target, num_cls=num_cls)))
                            losses_summary.append(criterions.DiceCoef(out_bs[modc], target, num_cls=num_cls))
                            kl_losses.append(criterions.temp_weighted_kl_loss(out_bs[modc], out_bs[4].detach(), target, num_cls=num_cls, temp=temp))
                            # cnt += 1
                        else:
                    ...
                    # kl0 = criterions.DiceKDLoss(out_bs[0], out_bs[4].detach(), target, num_cls=num_cls, temp=temp)
                    # kl1 = criterions.DiceKDLoss(out_bs[1], out_bs[4].detach(), target, num_cls=num_cls, temp=temp)
                    # kl2 = criterions.DiceKDLoss(out_bs[2], out_bs[4].detach(), target, num_cls=num_cls, temp=temp)
                    # kl3 = criterions.DiceKDLoss(out_bs[3], out_bs[4].detach(), target, num_cls=num_cls, temp=temp)
                    ...
                    loss_reg = 0.5 * term_kl + 0.02 * term_proto + 0.5 * prm_loss + torch.sum(torch.stack(losses, dim=0))
                    ...
                    msg += 'prm_loss:{:.4f}, '.format(prm_loss.item())
