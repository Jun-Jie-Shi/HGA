...
def main():
    ...
    if args.model == 'mmformer':
        ...
        model = DualNet_SS.DualNet_SS(args=args, norm_cfg='IN', activation_cfg='LeakyReLU',
    ...
    if args.use_passion:
        ...
        for epoch in range(args.num_epochs):
            ...
            for i in range(iter_per_epoch):
                ...
                N, C, D, W, H = x.shape
                # kl_loss = torch.zeros(N,4).float().cuda()
                kl0 = criterions.temp_kl_loss_bs(logits0, preds.detach(), target, num_cls=num_cls, temp=temp)
                kl1 = criterions.temp_kl_loss_bs(logits1, preds.detach(), target, num_cls=num_cls, temp=temp)
                kl2 = criterions.temp_kl_loss_bs(logits2, preds.detach(), target, num_cls=num_cls, temp=temp)
                kl3 = criterions.temp_kl_loss_bs(logits3, preds.detach(), target, num_cls=num_cls, temp=temp)
                kl_loss = torch.cat((kl0, kl1, kl2, kl3), dim=1)
                kl_loss_m = torch.sum(kl_loss * mask, dim=0)
                # term_kl = kl_loss_m.sum()

                proto0, dist0 = criterions.prototype_passion_loss_bs(fe0, fep.detach(), target, num_cls=num_cls, temp=temp)
                proto1, dist1 = criterions.prototype_passion_loss_bs(fe1, fep.detach(), target, num_cls=num_cls, temp=temp)
                proto2, dist2 = criterions.prototype_passion_loss_bs(fe2, fep.detach(), target, num_cls=num_cls, temp=temp)
                proto3, dist3 = criterions.prototype_passion_loss_bs(fe3, fep.detach(), target, num_cls=num_cls, temp=temp)

                proto_loss = torch.cat((proto0, proto1, proto2, proto3), dim=1)
                proto_loss_m = torch.sum(proto_loss * mask, dim=0)
                # term_proto = proto_loss_m.sum()

                dist_ = torch.cat((dist0, dist1, dist2, dist3), dim=1)
                dist_m_bs = mask * dist_
                dist_m = torch.sum(dist_*mask, dim=0)

                for bs in range(x.size(0)):
                    dist_avg_bs = sum(dist_m_bs[bs])/sum(mask[bs])
                    # rp_iter += mask[bs]*(dist_m_bs[bs]/(dist_avg_bs + 1e-8)-1)
                    rp_iter += mask[bs]*(1 - dist_m_bs[bs]/(dist_avg_bs + 1e-8))
                rp_mask = rp_iter > 0
                ...
                loss = term_seg_Dice + term_seg_BCE + 0.1 * term_shared + 0.02 * term_spec + 0.5 * term_kl + 0.1 * term_proto
