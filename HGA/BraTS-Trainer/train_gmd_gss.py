...
def main():
    ...
    if args.model == 'mmformer':
        ...
        model = ensemble_inc.Ensemble(in_channels=4, out_channels=num_cls, width_ratio=0.5, feature=True)
    ...
    if args.use_passion:
        ...
        for epoch in range(args.num_epochs):
            ...
            for i in range(iter_per_epoch):
                ...
                out_bs, bottle_fe_bs = model(x, mask, target=target)
                losses = []
                losses_summary = []
                ...
                # losses /= cnt
                flair_pred = torch.nn.Softmax(dim=1)(out_bs[0])
                t1ce_pred = torch.nn.Softmax(dim=1)(out_bs[1])
                t1_pred = torch.nn.Softmax(dim=1)(out_bs[2])
                t2_pred = torch.nn.Softmax(dim=1)(out_bs[3])
