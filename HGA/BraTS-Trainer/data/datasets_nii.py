...
class Brats_loadall_train_nii_idt_wimaskvalue(Dataset):
...
class Brats_loadall_metaval_nii_idt(Dataset):
    def __init__(self, transforms='', root=None, modal='all', num_cls=4, mask_value=14, train_file=None):
        self.excel_path = train_file
        self.mask_value = mask_value
        excel_data_all = pd.read_csv(self.excel_path)
        # excel_data = excel_data_all[excel_data_all['mask_id'] == mask_value]
...
class Brats_loadall_val_nii(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask = torch.squeeze(torch.from_numpy(mask_valid_array[3]), dim=0)
        return x, yo, mask, name
