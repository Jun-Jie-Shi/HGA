...
from models import rfnet_passionv2, mmformer, m2ftrans_passionv2, mmformer_ce, mmformer_gss, nnunet, mmformer_mag
...
def main():
    ...
    elif args.model == 'mmformer_gss':
        model = mmformer_gss.Model(num_cls=num_cls)
    elif args.model == 'mmformer_mag':
        model = mmformer_mag.Model(num_cls=num_cls)
    elif args.model == 'rfnet':
        model = rfnet_passionv2.Model(num_cls=num_cls)
    elif args.model == 'm2ftrans':
        model = m2ftrans_passionv2.Model(num_cls=num_cls)
    elif args.model == 'nnunet':
        model = nnunet.Generic_nnunet(num_classes=num_cls)
        ...
        if args.use_passion:
            ...
            for epoch in range(args.num_epochs):
                ...
                for i in range(iter_per_epoch):
                    ...
                    fuse_loss = torch.mean(fuse_loss_bs)

                    prm_loss = torch.mean(prm_loss_bs)
                    ...
                    if args.mask_type == 'pdt':
                        ...
                        if epoch < args.region_fusion_start_epoch:
                            ...
                            loss = fuse_loss + sep_loss + prm_loss + kl_loss * 0.1 + proto_loss * 0.1
                        ...
                        sep_loss_m = torch.mean(sep_loss_m_bs*mask, dim=0)
                        kl_loss_m = torch.mean(kl_loss_m_bs*mask, dim=0)
                        proto_loss_m = torch.mean(proto_loss_m_bs*mask, dim=0)
                        ...
                        # multi_loss = torch.sum(fuse_loss_bs, dim=0) + torch.sum(prm_loss_bs, dim=0)
                        # uni_m_loss = sep_loss_m + kl_loss_m * 0.5 + proto_loss_m * 0.1
                        # multi_uni_loss_bs = torch.cat((multi_loss,uni_m_loss), dim=0)

                        ## warmup with shared sep-decoder like rfnet
                        if epoch < args.region_fusion_start_epoch:
                            sep_loss = (imb_beta * modal_weight * sep_loss_m).sum()
                            loss = fuse_loss * 0.0 + sep_loss*0.0 + prm_loss * 0.0 + kl_loss * 0.0 + proto_loss * 0.0
                        else:
                            sep_loss = (modal_weight * sep_loss_m).sum()
                            loss = fuse_loss + sep_loss + prm_loss + kl_loss * 0.5 + proto_loss * 0.02
                            # loss = fuse_loss + prm_loss + kl_loss * 0.5 + proto_loss * 0.1

                    # ## without warmup and without shared sep-decoder
                    # sep_loss = (rp_mask * imb_beta * modal_weight * sep_loss_m).sum()
                    # loss = fuse_loss + sep_loss * 0.0 + prm_loss + kl_loss * 0.5 + proto_loss * 0.1

                    # pc_opt_enc.zero_grad()
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()

                    # optimizer.zero_grad()
                    # pc_opt_enc.pc_backward(list(torch.chunk(torch.cat((torch.sum(fuse_loss_bs, dim=0), sep_loss_m), dim=0), 5, dim=0)))
                    # pc_opt_enc.step()
                    # optimizer.step()
                    # pc_opt_enc.zero_grad()


                    # pc_opt_sep.zero_grad()
                    # pc_opt_sep.pc_backward(list(torch.chunk(sep_loss_m, 4, dim=0)))
                    # pc_opt_sep.step()
                    # pc_adam.zero_grad()
                    # pc_adam.pc_backward(list(torch.chunk(multi_uni_loss_bs, 5, dim=0)))
                    # pc_adam.step()
                ...
                if epoch < args.region_fusion_start_epoch:
                    ...
                    imb_beta = torch.clamp(imb_beta, min=args.beta_min, max=4.0)
            ...
            for epoch in range(args.num_epochs):
                ...
                for i in range(iter_per_epoch):
                    ...
                    if args.mask_type == 'pdt':
                        sep_loss_m = torch.mean(sep_loss_m_bs, dim=0)
