...
def main():
    ...
    if not args.use_passion:
        optimizer = torch.optim.AdamW(train_params,  betas=(0.9, 0.999), eps=1e-08, amsgrad=True)
        # pc_adam = PCGrad(optimizer, reduction='mean', writer=writer)
    else:
        optimizer = torch.optim.AdamW(train_params,  betas=(0.9, 0.999), eps=1e-08, amsgrad=True)
    # pc_adam = GMD(optimizer, reduction='sum', writer=writer)
    # params_enc = []
    # params_sep = []
    # params_fuse = []
    ...
    #     ####BRATS2021
    train_file = os.path.join(args.datarootPath, args.imbmrpath)
    test_file = os.path.join(args.datasetPath, args.test_txtname)

    logging.info(str(args))
    set_seed(args.seed)
    if not args.only_eval:
        if not args.use_passion:
            train_set = Brats_loadall_train_nii_idt_wimaskvalue(transforms=args.train_transforms, root=args.datasetPath, num_cls=num_cls, mask_type=args.mask_type, train_file=train_file)
        ...
        if args.use_passion:
            ...
            for epoch in range(args.num_epochs):
                ...
                for i in range(iter_per_epoch):
                    ...
                    if meta_mask_value_pre==14:
                        ...
                        # with torch.no_grad():
                        #     for p, g in zip(model.parameters(), phi_tilde):
                        #         # 原地计算：p = g + epsilon * (p - g)
                        #         p.data.sub_(g).mul_(epsilon).add_(g)  # 高效原地更新
                        #     # 原地更新 phi_tilde
                    ...
                    for modc in range(4):
                        if mask[0,modc]:
                            losses.append(criterions.DiceCoef(out_bs[modc], target, num_cls=num_cls) + criterions.softmax_weighted_loss(torch.nn.Softmax(dim=1)(out_bs[modc]), target, num_cls=num_cls))
                    ...
                    # losses = criterions.DiceCoef(out_bs[4], target, num_cls=num_cls)
                    # losses_summary.append(losses)
                    losses.append(criterions.DiceCoef(out_bs[4], target, num_cls=num_cls)+ criterions.softmax_weighted_loss(torch.nn.Softmax(dim=1)(out_bs[4]), target, num_cls=num_cls))
                    # cnt += 1
                    ...
                    loss_reg = torch.sum(torch.stack(losses, dim=0))
                    # losses /= cnt
            ...
            for epoch in range(args.num_epochs):
                ...
                for i in range(iter_per_epoch):
                    ...
                    # losses /= cnt
                    flair_pred, t1ce_pred, t1_pred, t2_pred = torch.nn.Softmax(dim=1)(out_bs[0]), torch.nn.Softmax(dim=1)(out_bs[1]), torch.nn.Softmax(dim=1)(out_bs[2]), torch.nn.Softmax(dim=1)(out_bs[3])
                    ...
                    for modc in range(4):
                        if mask[0,modc]:
                            ...
                            losses_summary.append(criterions.softmax_weighted_loss(torch.nn.Softmax(dim=1)(out_bs[modc]), target, num_cls=num_cls) + criterions.dice_loss(torch.nn.Softmax(dim=1)(out_bs[modc]), target, num_cls=num_cls)+criterions.temp_kl_loss(torch.nn.Softmax(dim=1)(out_bs[modc]), teacher_pred.detach(), target, num_cls=num_cls))
                            # cnt += 1
                        else:
                            losses_summary.append(torch.tensor(0).cuda().float())
                    losses_summary.append(criterions.softmax_weighted_loss(torch.nn.Softmax(dim=1)(out_bs[4]), target, num_cls=num_cls)+ criterions.dice_loss(torch.nn.Softmax(dim=1)(out_bs[4]), target, num_cls=num_cls))
                    # losses = criterions.DiceCoef(out_bs[4], target, num_cls=num_cls)
                    # losses_summary.append(losses)
                    # cnt += 1
                    # logging.info(losses_summary)
