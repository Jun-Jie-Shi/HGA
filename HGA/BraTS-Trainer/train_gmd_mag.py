    ...
    if args.use_passion:
        ...
        for epoch in range(args.num_epochs):
            ...
            for i in range(iter_per_epoch):
                ...
                fkl0 = torch.nn.L1Loss()(bottle_fe_bs[0], bottle_fe_bs[4])
                fkl1 = torch.nn.L1Loss()(bottle_fe_bs[1], bottle_fe_bs[4])
                fkl2 = torch.nn.L1Loss()(bottle_fe_bs[2], bottle_fe_bs[4])
                fkl3 = torch.nn.L1Loss()(bottle_fe_bs[3], bottle_fe_bs[4])

                term_kl = (kl_loss_m).sum()
                term_fkl = fkl0 + fkl1 + fkl2 + fkl3

                loss = torch.sum(torch.stack(losses, dim=0)) + 0.5 * term_kl + 0.01 * term_fkl
