...
def main():
    ...
    # device = model.device
    gn_adam = GradNorm(optimizer, n_tasks=4, writer=writer)
        ...
        if args.use_passion:
                ...
                for i in range(iter_per_epoch):
                    ...
                    for modc in range(4):
                        if mask[0,modc]:
                            ...
                            losses_summary.append(criterions.DiceCoef(out_bs[modc], target, num_cls=num_cls)+ criterions.softmax_weighted_loss(torch.nn.Softmax(dim=1)(out_bs[modc]), target, num_cls=num_cls))
                            # cnt += 1
                        else:
                    ...
                    gn_adam.zero_grad()
                    gn_adam.gradnorm_backward(losses_summary, losses[0])
                    gn_adam.step()
            ...
            for epoch in range(args.num_epochs):
                ...
                for i in range(iter_per_epoch):
                    ...
                    gn_adam.zero_grad()
                    gn_adam.gradnorm_backward(losses_summary)
                    gn_adam.step()
