...
def cal_hd95_myops(output, target):
    # LVB
    mask_gt = (target == 2).astype(int)
    mask_pred = (output == 2).astype(int)
    hd95_lvb = compute_BraTS_HD95(mask_gt, mask_pred)
    del mask_gt, mask_pred

    # rvb
    mask_gt = (target == 3).astype(int)
    mask_pred = (output == 3).astype(int)
    hd95_rvb = compute_BraTS_HD95(mask_gt, mask_pred)
    del mask_gt, mask_pred

    # MYO
    mask_gt = ((target == 1) | (target ==4) | (target ==5)).astype(int)
    mask_pred = ((output == 1) | (output ==4) | (output ==5)).astype(int)
    hd95_myo = compute_BraTS_HD95(mask_gt, mask_pred)
    del mask_gt, mask_pred

    return (hd95_lvb, hd95_rvb, hd95_myo)

def softmax_output_dice_class6_myops(output, target):
    eps = 1e-8
    #######label1########
    o1 = (output == 1).float()
    t1 = (target == 1).float()
    ...
    normal_myo_dice = intersect1 / denominator1

    o2 = (output == 2).float()
    t2 = (target == 2).float()
    intersect2 = torch.sum(2 * (o2 * t2), dim=(1,2,3)) + eps
    denominator2 = torch.sum(o2, dim=(1,2,3)) + torch.sum(t2, dim=(1,2,3)) + eps
    lvb_dice = intersect2 / denominator2

    o3 = (output == 3).float()
    t3 = (target == 3).float()
    intersect3 = torch.sum(2 * (o3 * t3), dim=(1,2,3)) + eps
    denominator3 = torch.sum(o3, dim=(1,2,3)) + torch.sum(t3, dim=(1,2,3)) + eps
    rvb_dice = intersect3 / denominator3

    o4 = (output == 4).float()
    t4 = (target == 4).float()
    intersect4 = torch.sum(2 * (o4 * t4), dim=(1,2,3)) + eps
    denominator4 = torch.sum(o4, dim=(1,2,3)) + torch.sum(t4, dim=(1,2,3)) + eps
    edema_myo_dice = intersect4 / denominator4

    o5 = (output == 5).float()
    t5 = (target == 5).float()
    intersect5 = torch.sum(2 * (o5 * t5), dim=(1,2,3)) + eps
    denominator5 = torch.sum(o5, dim=(1,2,3)) + torch.sum(t5, dim=(1,2,3)) + eps
    scars_myo_dice = intersect5 / denominator5

    o_myo = o1 + o4 + o5
    t_myo = t1 + t4 + t5
    intersect_myo = torch.sum(2 * (o_myo * t_myo), dim=(1,2,3)) + eps
    denominator_myo = torch.sum(o_myo, dim=(1,2,3)) + torch.sum(t_myo, dim=(1,2,3)) + eps
    dice_myo = intersect_myo / denominator_myo


    dice_separate = torch.cat((torch.unsqueeze(normal_myo_dice, 1), torch.unsqueeze(lvb_dice, 1), torch.unsqueeze(rvb_dice, 1), torch.unsqueeze(edema_myo_dice, 1), torch.unsqueeze(scars_myo_dice, 1)), dim=1)
    dice_evaluate = torch.cat((torch.unsqueeze(lvb_dice, 1), torch.unsqueeze(rvb_dice, 1), torch.unsqueeze(dice_myo, 1)), dim=1)
...
def test_dice_hd95_softmax_myops(
        test_loader,
        model,
        dataname = '/data/MyoPS/MyoPS2020',
        feature_mask=None,
        mask_name=None,
        csv_name=None,
        ):
    ...
    if dataname in ['/data/MyoPS/MyoPS2020']:
        num_cls = 6
        class_evaluation= 'lvb', 'rvb', 'myo'
        class_separate = 'normal_myo', 'lvb', 'rvb', 'edema', 'scars'
    ...
    for i, data in enumerate(test_loader):
        ...
        if dataname in ['/data/MyoPS/MyoPS2020']:
            scores_separate, scores_evaluation = softmax_output_dice_class6_myops(pred, target)
            scores_hd95 = np.array(cal_hd95_myops(pred[0].cpu().numpy(), target[0].cpu().numpy()))
        ...
        for k, name in enumerate(names):
            ...
            csv_writer.writerow([scores_evaluation[k][0], scores_evaluation[k][1], scores_evaluation[k][2],\
                scores_hd95[0], scores_hd95[1], scores_hd95[2]])
            file.close()
