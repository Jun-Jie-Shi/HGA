...
from utils.criterions import temp_kl_loss, softmax_weighted_loss, dice_loss, prototype_loss, prototype_pmr_loss
    ...
    def forward(self, x, mask, target=None, temp=1.0):
        ...
        if self.is_training:
            ...
            if mask[0,0]:
                ...
                proto_loss[0], fkl_loss[0] = prototype_pmr_loss(de_f_bssfp[0], de_f_avg[0].detach(), target, fuse_pred_bssfp, fuse_pred.detach(), num_cls=num_cls, temp=temp)
                kl_loss[0] += temp_kl_loss(fuse_pred_bssfp, fuse_pred.detach(), target, num_cls=num_cls, temp=temp)
                weight_prm = 1.0
                for prm_pred, prm_pred_bssfp, up_op in zip(preds, preds_bssfp, self.up_ops):
                    weight_prm /= 2.0
                    kl_loss[0] += weight_prm * temp_kl_loss(prm_pred_bssfp, prm_pred.detach(), target, num_cls=num_cls, temp=temp, up_op=up_op)
            if mask[0,1]:
                ...
                proto_loss[1], fkl_loss[1] = prototype_pmr_loss(de_f_lge[0], de_f_avg[0].detach(), target, fuse_pred_lge, fuse_pred.detach(), num_cls=num_cls, temp=temp)
                kl_loss[1] += temp_kl_loss(fuse_pred_lge, fuse_pred.detach(), target, num_cls=num_cls, temp=temp)
                weight_prm = 1.0
                for prm_pred, prm_pred_lge, up_op in zip(preds, preds_lge, self.up_ops):
                    weight_prm /= 2.0
                    kl_loss[1] += weight_prm * temp_kl_loss(prm_pred_lge, prm_pred.detach(), target, num_cls=num_cls, temp=temp, up_op=up_op)
            if mask[0,2]:
                ...
                proto_loss[2], fkl_loss[2] = prototype_pmr_loss(de_f_t2[0], de_f_avg[0].detach(), target, fuse_pred_t2, fuse_pred.detach(), num_cls=num_cls, temp=temp)
                kl_loss[2] += temp_kl_loss(fuse_pred_t2, fuse_pred.detach(), target, num_cls=num_cls, temp=temp)
                weight_prm = 1.0
                for prm_pred, prm_pred_t2, up_op in zip(preds, preds_t2, self.up_ops):
                    weight_prm /= 2.0
                    kl_loss[2] += weight_prm * temp_kl_loss(prm_pred_t2, prm_pred.detach(), target, num_cls=num_cls, temp=temp, up_op=up_op)
            weight_prm = 1.0
