...
# get_local.activate()
from models import mmformer_2d_gmd
...
parser.add_argument('--savepath', default='outputs/idt_mr537_meta_imb_ra_reptile_linear_gmd_bs1_epoch300_lr2e-4_temp4', type=str)
...
def main():
    ...
    if args.model == 'mmformer_2d_passion':
        model = mmformer_2d_gmd.Model(num_cls=num_cls)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(iter_per_epoch):
            ...
            for modc in range(3):
                if mask[0,modc]:
                    losses.append(sep_loss_m[modc])
            losses.append(fuse_loss)
