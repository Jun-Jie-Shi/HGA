...
from data.datasets_nii import (Myops_loadall_test_nii, Myops_loadall_train_nii_longtail, Myops_loadall_val_nii)
...
parser.add_argument('--imbmrpath', default='/data/MyoPS/myops_split/MyoPS2020_longtail_split_357.csv', type=str, help='csv path')
parser.add_argument('--savepath', default='outputs/idt_mr357_meta_imb_ra_reptile_linear_passion2_bs1_epoch300_lr2e-3_temp4', type=str)
...
def main():
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(iter_per_epoch):
            ...
            if max(fkl_loss_m/fkl_avg -1)<0.2:
                ra_mask = mask[0]*(fkl_loss_m/fkl_avg-1) > -0.2
            else:
                ra_mask = mask[0]*(fkl_loss_m/fkl_avg-1) > 0
            ...
            msg += 'fkllist:[{:.4f},{:.4f},{:.4f}] '.format(fkl_loss_m[0].item(), fkl_loss_m[1].item(), fkl_loss_m[2].item())
            msg += '{:>20}, '.format(name[0])
            msg += 'kl_w[{:.2f},{:.2f},{:.2f}] '.format(modal_weight[0].item(), modal_weight[1].item(), modal_weight[2].item())
        ...
        for m in range(3):
            ...
            writer.add_scalar('fkl_m{}'.format(m), epoch_fkl_m[m].item(), global_step=(epoch+1))
            writer.add_scalar('ra_m{}'.format(m), ra[m].item(), global_step=(epoch+1))
