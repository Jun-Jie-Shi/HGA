...
class UncertaintyWeight:
    ...
    def step(self):
        ...
        with torch.no_grad():
            ...
            logging.info('Modality Weights:[{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}]'.format(self.last_weights[0].item(),self.last_weights[1].item(),self.last_weights[2].item(),self.last_weights[3].item(),self.last_weights[4].item()))
