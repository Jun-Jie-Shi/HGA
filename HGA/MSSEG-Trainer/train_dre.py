...
from data.datasets_nii import (MSSEG_loadall_test_nii, MSSEG_loadall_train_nii_idt, MSSEG_loadall_metaval_nii_idt, MSSEG_loadall_train_nii_idt_wimaskvalue, MSSEG_loadall_val_nii, MSSEG_loadall_val_nii_idt)
...
parser.add_argument('--savepath', default='outputs/idt_mr97531_dre_hga_bs4_epoch300_lr2e-4_temp4_seed1038', type=str)
...
def main():
    ...
    dre_adam = DRe(optimizer, n_tasks=5, writer=writer)
    temp = args.temp
    ##########Setting data
        ####BRATS2020
    ...
    set_seed(args.seed)
    train_setd = MSSEG_loadall_metaval_nii_idt(transforms=args.train_transforms, root=args.datapath, mask_value=30, num_cls=num_cls, train_file=train_file)
    valid_setv = MSSEG_loadall_val_nii_idt(transforms=args.train_transforms, root=args.datapath, num_cls=num_cls, train_file=valid_file)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(iter_per_epoch):
            if not dre_adam.check_relearning():
                ...
                objs_dv=[]
                for modc in range(5):
                    objs_dv.append(criterions.DiceCoef2(out_dv_bs[modc], target_dv, num_cls=num_cls)+ criterions.softmax_weighted_loss(torch.nn.Softmax(dim=1)(out_dv_bs[modc]), target_dv, num_cls=num_cls))
